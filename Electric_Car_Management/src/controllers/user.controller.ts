...
export async function updateUserInfo(req: Request, res: Response) {
  try {
    ...
    if (req.file) {
      try {
        const uploadResult = await uploadService.uploadImage(req.file.buffer);
  ...
  try {
    ...
    if (!id) {
      return res.status(403).json({
        message: "Vui lòng cập nhật số điện thoại trước khi tạo bài viết",
      });
    }
...
export async function getUserPosts(req: Request, res: Response) {
  try {
    ...
    const posts = await getPostByUserId(id, status, status_verify, page, limit);
