...
export function setupAuctionSocket() {
  ...
  auctionNamespace.on("connection", (socket) => {
    ...
    socket.on("auction:join", async ({ auctionId }) => {
      try {
        const auction = await auctionService.getAuctionExisting(auctionId);
        if (!auction) {
        ...
        const auctionStatus = await auctionService.getAuctionStatus(auctionId);
        const remainingTime = await auctionService.getAuctionRemainingTime(
          auctionId
        );
    ...
    socket.on(
      ...
      async (data: { auctionId: number; bidAmount: number }) => {
        try {
          ...
          auctionNamespace.to(privateRoom).emit("auction:bid_update", {
            auctionId,
            winnerId: userId,
            winningPrice: bidAmount,
            message: result.message,
    ...
    socket.on("disconnect", () => {
      console.log(`❌ User ${userId} disconnected from auction namespace`);
    });
  });
